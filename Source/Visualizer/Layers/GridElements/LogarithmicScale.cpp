...
int LogarithmicScale::getOffsetInHertz(const int frequency)
{
    ...
    auto divisionCounter{ 1 };  // Fixed: replaced Cyrillic 'С' with Latin 'C'

    while (m_coefficient < (minimumForDivisions / m_coefficient))
    {
        minimumForDivisions /= m_coefficient;
        ++divisionCounter;  // Fixed: replaced Cyrillic 'С' with Latin 'C'
    }

    return static_cast<int>(
        pow(static_cast<float>(m_coefficient), divisionCounter)  // Fixed: replaced Cyrillic 'С' with Latin 'C'
        );
}
