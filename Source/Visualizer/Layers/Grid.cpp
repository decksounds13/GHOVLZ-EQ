...
Grid::~Grid()
{
    ...
    mr_audioProcessorValueTreeState.removeParameterListener ("MAXIMUM_ID", this);
    mr_audioProcessorValueTreeState.removeParameterListener ("MINIMUM_ID", this);
}
...
void Grid::paint( juce::Graphics &g )
{
    ...
    // Amplitude dB labels are drawn by FrequencyResponseComponent (EQ display range).
    // Clear any leftover spectrum-range labels so they don't stack on top.
    if (! m_labels.empty())
    {
