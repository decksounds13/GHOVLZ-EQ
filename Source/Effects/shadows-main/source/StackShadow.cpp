...
void StackShadow::blurImage(juce::Image& img, unsigned int radius)
{
    applyStackBlur(img, radius);
}
...
void StackShadow::drawFixedOuterShadowForPath(juce::Graphics& g, const juce::Path& path) const
{
    jassert(blur > 0);

    auto area = path.getBounds()
        .expanded(blur + 1)
        .getIntersection(g.getClipBounds().toFloat().expanded(blur + 1));
